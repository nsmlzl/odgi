                        ...
                        [&](uint64_t tid) {
                            ...
                            while (work_todo.load()) {
                                if (!snapshot_in_progress.load()) {
                                    ...
                                    int INTERVAL = 10;
                                    // simply select step_b as the next step in the path
                                    if (s_rank < path_step_count - INTERVAL) {
                                        as_integers(step_b)[0] = path_i;
                                        as_integers(step_b)[1] = s_rank + INTERVAL;
                                    } else {
                                        // or the previous step in the path
                                        as_integers(step_b)[0] = path_i;
                                        as_integers(step_b)[1] = s_rank - INTERVAL;
                                    }
